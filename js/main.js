// --- START OF FULL main.js (Phase Logic Removed, Corrected Full File, New Listeners) ---
...
function initializeApp() {
    ...
    UI.updateInsightDisplays(); // Includes boost button update
...
function attachEventListeners() {
    ...
    // Combined close button listener
    ...
    if (grimoireContent) { // Card Area
        grimoireContent.addEventListener('click', (event) => { // Button Clicks
         ...
         grimoireContent.addEventListener('dragstart', (event) => { // Drag Start
             const card = event.target.closest('.concept-card');
             if (card && card.draggable) { event.dataTransfer.setData('text/plain', card.dataset.conceptId); event.dataTransfer.effectAllowed = 'move'; card.classList.add('dragging'); draggedCardId = parseInt(card.dataset.conceptId); console.log(`Dragging card: ${draggedCardId}`); }
             else { event.preventDefault(); }
         });
         grimoireContent.addEventListener('dragend', (event) => { // Drag End
    ...
    if (grimoireShelvesContainer) { // Shelves Container
        ...
        grimoireShelvesContainer.addEventListener('dragover', (event) => { // Drag Over Shelf
        ...
        grimoireShelvesContainer.addEventListener('dragleave', (event) => { // Drag Leave Shelf
        ...
        grimoireShelvesContainer.addEventListener('drop', (event) => { // Drop on Shelf
    ...
    if (personaScreenDiv) {
         ...
         // Listen for core action buttons
         personaScreenDiv.addEventListener('click', (event) => {
             if (event.target.matches('#elementalDilemmaButton')) GameLogic.handleElementalDilemmaClick(); // UPDATED ID
             ...
             else if (event.target.matches('#addInsightButton')) GameLogic.handleInsightBoostClick(); // ADDED Listener
         });
    }
    ...
    if (conceptDetailPopupElem) {
        conceptDetailPopupElem.addEventListener('click', (event) => {
            ...
            // else if (button.id === 'evolveArtButton') { /* REMOVED */ }
    ...
    // --- Resonance Chamber (Tapestry Deep Dive) Modal Actions (Delegation) --- NEW
        ...
        if (deepDiveDetail) {
            deepDiveDetail.addEventListener('click', (event) => {
                if (event.target.matches('#completeContemplationBtn')) {
                    // Need to retrieve the task data somehow, maybe store it temporarily when displayed
                    console.warn("Complete Contemplation button clicked, but task data retrieval not fully implemented in this example.");
                    // GameLogic.handleCompleteContemplation(retrievedTaskData);
                }
            });
        }
    }

    // --- Elemental Dilemma Modal --- NEW
    if (dilemmaModalElem) {
