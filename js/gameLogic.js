// --- START OF FULL gameLogic.js (Phase Checks Removed, Corrected Full File) ---
    ...
    if (State.unlockLoreLevel(conceptId, level)) {
        ...
        if (getCurrentPopupConceptId() === conceptId && document.getElementById('conceptDetailPopup')) {
            requestAnimationFrame(() => {
                ...
                // ... (Keep the debugging counters and arrays for now if you want) ...

                if (loreContentContainer) {
                    // ... (Keep the loop to find loreEntryDiv) ...
                ...
                if (loreEntryDiv) { // If found within rAF
                    ...
                    if (loreData) {
                        ...
                        `;
                        // *** ADD THIS LINE: Re-apply the dataset attribute ***
                        loreEntryDiv.dataset.loreLevel = level;
                        // *****************************************************

                        console.log(`rAF: Updated DOM successfully for level ${level} and re-applied data attribute.`);
                         ...
                         if (cardElemIndicator) {
                            ...
                            const currentlyUnlocked = State.getUnlockedLoreLevel(conceptId);
...
export function calculateTapestryNarrative(forceRecalculate = false) {
    // Get the hash stored in the state
    const stateHash = State.getState().currentFocusSetHash;
    // Calculate the hash based on the *current* actual focus set
