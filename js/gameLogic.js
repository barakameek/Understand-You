// --- START OF FULL gameLogic.js --- (Combined Corrections, Categories, Lore, Synergy Button Logic)
...
export function finalizeQuestionnaire() {
    ...
    State.setQuestionnaireComplete(); // Marks done, advances phase, saves
    ...
    UI.displayPersonaSummary(); // This now includes drawing the chart
...
export function determineStarterHandAndEssence() {
    ...
    // Ensure some elemental diversity
...
export function handleResearchClick({ currentTarget, isFree = false }) {
    ...
    const cost = parseFloat(button.dataset.cost); // Get cost from element data attribute
    ...
    if (isFree) {
        if (State.useInitialFreeResearch()) { // useInitial... handles state saving
            console.log(`Used initial free research on ${elementKey}.`);
            conductResearch(elementKey);
            UI.displayStudyScreenContent(); // Re-render to update free count / costs
        } else {
            console.warn("Attempted free research, but none remaining.");
            UI.showTemporaryMessage("No free research attempts left.", 3000);
        }
    } else {
        if (spendInsight(cost, `Research: ${elementKeyToFullName[elementKey]}`)) { // spendInsight handles state saving
            ...
            UI.displayStudyScreenContent(); // Re-render to update button states if insight low
        }
    }
}
...
export function handleFreeResearchClick() {
    ...
    State.setFreeResearchUsed(); // Handles state saving
    UI.displayStudyScreenContent(); // Update button states
...
export function conductResearch(elementKeyToResearch) {
    ...
    const researchOutElem = document.getElementById('studyResearchDiscoveries');
...
export function addConceptToGrimoireById(conceptId, buttonElement = null) {
    ...
    if (distance > Config.DISSONANCE_THRESHOLD && State.getOnboardingPhase() >= Config.ONBOARDING_PHASE.REFLECTION_RITUALS) {
        ...
        addConceptToGrimoireInternal(conceptId, context); // Pass context
    }
 }
...
export function addConceptToGrimoireInternal(conceptId, context = 'unknown') { // context added
     ...
     if (State.addDiscoveredConcept(conceptId, conceptToAdd)) { // State now initializes lore/category
         ...
         if (conceptToAdd.relatedIds && conceptToAdd.relatedIds.length > 0) {
             ...
             for (const relatedId of conceptToAdd.relatedIds) {
                 if (discoveredMap.has(relatedId)) { bonusInsight += Config.SYNERGY_INSIGHT_BONUS; if (!synergyMessageShown) { const relatedConcept = discoveredMap.get(relatedId)?.concept; UI.showTemporaryMessage(`Synergy Bonus: +${Config.SYNERGY_INSIGHT_BONUS.toFixed(1)} Insight (Related to ${relatedConcept?.name || 'a known concept'})`, 3500); synergyMessageShown = true; } }
             }
         ...
         if (context === 'discovery') { updateMilestoneProgress('addToGrimoireFromResearch', 1); console.log("Tracked addToGrimoireFromResearch"); }
...
export function handleToggleFocusConcept() {
    ...
    UI.updateFocusButtonStatus(currentlyDisplayedConceptId); // Update popup button state
}

export function handleCardFocusToggle(conceptId) {
    if (isNaN(conceptId)) return;
    const result = State.toggleFocusConcept(conceptId); // Handles state, phase, saving
    ...
    if (result === 'not_discovered') { UI.showTemporaryMessage("Concept not in Grimoire.", 3000); }
        ...
        UI.refreshGrimoireDisplay(); // Update card appearance
        calculateTapestryNarrative(true); // Recalculate narrative
        checkSynergyTensionStatus(); // Update synergy button state

        // Update Persona screen displays if visible
        ...
        checkForFocusUnlocks(); // Check synergy unlocks
        UI.updateTapestryDeepDiveButton();
        UI.updateSuggestSceneButtonState();

        if (currentlyDisplayedConceptId === conceptId) { UI.updateFocusButtonStatus(conceptId); } // Update popup button if open
    }
}
...
export function sellConcept(conceptId, context) {
    ...
    if (confirm(`Sell '${concept.name}' (${concept.rarity}) from ${sourceLoc} for ${sellVal.toFixed(1)} Insight?`)) {
        ...
        else if (context === 'discovery') { UI.updateResearchButtonAfterAction(conceptId, 'sell'); }
...
export function triggerReflectionPrompt(context = 'Standard', targetId = null, category = null) {
    ...
    if (!selPrompt) {
        if (context === 'Dissonance' && targetId) { title = "Dissonance Reflection"; const concept = concepts.find(c => c.id === targetId); promptCatLabel = concept ? concept.name : "Dissonant Concept"; promptPool = reflectionPrompts.Dissonance || []; showNudge = true; reward = 5.0; console.log(`Looking for Dissonance prompt for ${promptCatLabel}`); }
        else if (context === 'Guided' && category) { title = "Guided Reflection"; promptCatLabel = category; promptPool = reflectionPrompts.Guided?.[category] || []; reward = Config.GUIDED_REFLECTION_COST + 2; console.log(`Looking for Guided prompt: ${category}`); }
    ...
    else { console.error(`Could not select prompt for ${context}`); if (context === 'Dissonance' && reflectionTargetConceptId) { console.warn("Dissonance reflection fail, adding concept."); addConceptToGrimoireInternal(reflectionTargetConceptId); UI.hidePopups(); UI.showTemporaryMessage("Reflection unavailable, added concept.", 3500); } else if (context === 'Guided') { gainInsight(Config.GUIDED_REFLECTION_COST, "Refund: No guided prompt"); UI.showTemporaryMessage("No guided reflections available.", 3000); } else { UI.showTemporaryMessage("No reflection prompt found.", 3000); } clearPopupState(); }
}
...
export function handleConfirmReflection(nudgeAllowed) {
    ...
    if (currentReflectionContext === 'Dissonance') {
        ...
        if (nudgeAllowed && reflectionTargetConceptId) {
         ...
         if (reflectionTargetConceptId) { addConceptToGrimoireInternal(reflectionTargetConceptId, 'dissonance'); }
    }
...
export function attemptArtEvolution() {
    ...
    const cost = Config.ART_EVOLVE_COST; const isFocused = State.getFocusedConcepts().has(conceptId); const hasReflected = State.getSeenPrompts().size > 0; const phaseOK = State.getOnboardingPhase() >= Config.ONBOARDING_PHASE.ADVANCED;
...
function unlockDeepDiveLevelInternal(elementKey, levelToUnlock) {
    ...
    if (spendInsight(cost, `Unlock Library: ${elementKeyToFullName[elementKey]} Lv ${levelToUnlock}`)) {
        if (State.unlockLibraryLevel(elementKey, levelToUnlock)) { // unlockLibraryLevel handles phase/save
...
function attemptExperimentInternal(experimentId) {
     ...
     if (roll < (exp.successRate || 0.5)) {
         ...
         const addedRepo = State.addRepositoryItem('experiments', exp.id);
     ...
     UI.displayRepositoryContent(); // Refresh repo display
}
...
export function handleSuggestSceneClick() {
     ...
     if (relevantUndiscoveredScenes.length === 0) { UI.showTemporaryMessage("All relevant scenes for this focus have been discovered. Check Repository.", 3500); }
     else {
         ...
         } else { console.error(`Failed to add scene ${selectedScene.id} to repository state.`); gainInsight(cost, "Refund: Scene Suggestion Error (State Add Fail)"); UI.showTemporaryMessage("Error suggesting scene.", 3000); }
     }
 }

// --- ** NEW: Grimoire Category & Lore Logic ** ---
...
export function handleCategorizeCard(conceptId, categoryId) {
    ...
    if (State.setCardCategory(conceptId, categoryId)) {
        ...
        UI.refreshGrimoireDisplay({ filterCategory: currentFilterCategory }); // Refresh display
        checkCategoryUnlocks(categoryId); // Check unlocks
        checkAndUpdateRituals('categorizeCard', { categoryId: categoryId, conceptId: conceptId });

        // Advance phase on first categorization (to enable Reflection/Rituals etc.)
        const anyCategorized = Array.from(State.getDiscoveredConcepts().values()).some(data => data.userCategory && data.userCategory !== 'uncategorized');
        if (anyCategorized && State.getOnboardingPhase() < Config.ONBOARDING_PHASE.STUDY_INSIGHT) { // Should advance to STUDY_INSIGHT (Phase 2)
             State.advanceOnboardingPhase(Config.ONBOARDING_PHASE.STUDY_INSIGHT);
...
function checkCategoryUnlocks(categoryId) {
    ...
    console.log(`Checking category unlocks for category: ${categoryId}`);
    const discoveredMap = State.getDiscoveredConcepts();
    ...
    categoryDrivenUnlocks.forEach(unlock => {
        if (unlock.categoryRequired === categoryId ) {
            ...
            if (!unlock.requiredInSameCategory || unlock.requiredInSameCategory.length === 0) requirementsMet = false;
            else { for (const reqId of unlock.requiredInSameCategory) { if (!cardIdSetInCategory.has(reqId)) { requirementsMet = false; break; } } }

            if (requirementsMet) {
                console.log(`Category unlock triggered: ${unlock.id}`);
                ...
                // Add State.markCategoryUnlockComplete(unlock.id) here if needed
            }
        }
    });
}
...
export function handleUnlockLore(conceptId, level, cost) {
    console.log(`Attempting to unlock lore level ${level} for concept ${conceptId} (Cost: ${cost})`);
    ...
    if (spendInsight(cost, `Unlock Lore: ${concept.name} Lvl ${level}`)) {
        unlockLoreInternal(conceptId, level, `Insight Purchase`); // Call internal helper
    }
}
...
export function checkForDailyLogin() {
    ...
    if (last !== today) {
        ...
        State.resetDailyRituals(); // Handles state save
        gainInsight(5.0, "Daily Bonus");
        UI.showTemporaryMessage("Rituals Reset. Free Research Available!", 3500);
        UI.displayDailyRituals();
        // Re-render relevant screens if they are visible
        if(document.getElementById('studyScreen')?.classList.contains('current')) UI.displayStudyScreenContent();
    } else {
        console.log("Already logged in today.");
        if(document.getElementById('studyScreen')?.classList.contains('current')) UI.displayStudyScreenContent(); // Ensure button state correct
    }
}
