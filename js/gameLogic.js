...
function determineStarterHandAndEssence() {
    ...
    if (conceptsWithDistance.length === 0) {
         console.error("Distance calculation failed."); const defaultStarters = concepts.slice(0, 5);
...
export function addConceptToGrimoireInternal(conceptId) {
     ...
     if (State.addDiscoveredConcept(concept.id, concept)) {
         ...
         if (currentlyDisplayedConceptId === conceptId) { UI.updateGrimoireButtonStatus(conceptId, false); UI.updateFocusButtonStatus(conceptId); UI.updatePopupSellButton(conceptId, concept, true, false); const notesSect = document.getElementById('myNotesSection'); if(notesSect && State.getOnboardingPhase() >= Config.ONBOARDING_PHASE.STUDY_INSIGHT) notesSect.classList.remove('hidden'); const evoSec = document.getElementById('popupEvolutionSection'); if(evoSec && State.getOnboardingPhase() >= Config.ONBOARDING_PHASE.ADVANCED) UI.displayEvolutionSection(concept, State.getDiscoveredConceptData(conceptId)); }
          UI.updateResearchButtonAfterAction(conceptId, 'add'); checkTriggerReflectionPrompt('add'); updateMilestoneProgress('addToGrimoire', 1); updateMilestoneProgress('discoveredConcepts.size', State.getDiscoveredConcepts().size);
...
function startReflectionCooldown() { if (reflectionCooldownTimeout) clearTimeout(reflectionCooldownTimeout); reflectionCooldownTimeout = setTimeout(() => { State.clearReflectionCooldown(); console.log("Reflection cooldown ended."); reflectionCooldownTimeout = null; }, 1000 * 60 * 3); } // 3 min
...
export function handleConfirmReflection(nudgeAllowed) {
    ...
    } else if (currentReflectionContext === 'Guided') { rewardAmt = Config.GUIDED_REFLECTION_COST + 2; attuneKey = null; }
    else if (currentReflectionContext === 'RareConcept') { rewardAmt = 7.0; const cEntry = Array.from(State.getDiscoveredConcepts().entries()).find(([id, data]) => data.concept.uniquePromptId === currentPromptId); attuneKey = cEntry ? cEntry[1].concept.primaryElement : null; }
    else if (currentReflectionContext === 'SceneMeditation') { const scene = sceneBlueprints.find(s => s.reflectionPromptId === currentPromptId); rewardAmt = (scene?.meditationCost || Config.SCENE_MEDITATION_BASE_COST) + 5; attuneKey = scene?.element || null; }
    else if (currentReflectionContext === 'Standard') { attuneKey = elementNameToKey[currentReflectionCategory] || null; if (!attuneKey) console.warn(`Key missing for standard reflection: ${currentReflectionCategory}`); }
...
export function triggerGuidedReflection() {
     if (spendInsight(Config.GUIDED_REFLECTION_COST, "Guided Reflection")) {
         ...
         else { console.warn("No guided categories."); gainInsight(Config.GUIDED_REFLECTION_COST, "Refund: No guided"); UI.showTemporaryMessage("No guided reflections available.", 3000); }
     }
}
...
export function checkAndUpdateRituals(action, details = {}) { // Corrected SINGLE definition
    ...
    currentRitualPool.forEach(ritual => {
        ...
        const actionMatch = ritual.track.action === action; const contextMatch = ritual.track.contextMatch && details.contextMatch === ritual.track.contextMatch;
...
export function checkForDailyLogin() {
    ...
    else { console.log("Already logged in."); UI.displayResearchButtons(); }
}
