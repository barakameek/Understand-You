...
export function updateResearchButtonAfterAction(conceptId, action) { const itemDiv = researchOutput?.querySelector(`.research-result-item[data-concept-id="${conceptId}"]`); if (!itemDiv) return; const addButton = itemDiv.querySelector('.add-button'); const sellButton = itemDiv.querySelector('.sell-button'); if (action === 'add') { if (addButton) { addButton.textContent = "In Grimoire"; addButton.disabled = true; } if (sellButton) sellButton.remove(); } else if (action === 'sell') { itemDiv.remove(); if (researchOutput && researchOutput.children.length === 0) { researchOutput.innerHTML = '<p><i>Research results cleared.</i></p>'; displayResearchStatus("Ready for new research."); } } }
