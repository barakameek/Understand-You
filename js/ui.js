// --- START OF FILE ui.js ---
// js/ui.js - UI Rendering & Interactions (Enhanced v4.10 + XP/Leveling v1.0 - Reconstructed)

import * as State from './state.js';
import * as Config from './config.js';
import * as Utils from './utils.js';
import * as GameLogic from './gameLogic.js'; // GameLogic reference might be needed for some UI updates based on its state/calculations
import {
    ...
    elementInteractionThemes, cardTypeThemes, createDefaultCardUnlockState // Import needed for popup display logic
} from '../data.js';

console.log("ui.js loading... (Enhanced v4.10 + XP/Leveling v1.0 - Reconstructed)");
...
const popupLoreSection = getElement('popupLoreSection'); // Legacy Lore section
const popupLoreContent = popupLoreSection?.querySelector('.collapsible-content'); // Content within legacy lore
...
// Internal Popup Display Helpers
...
function displayPopupLore(conceptId, loreData, unlockedLevel) { // Kept for legacy support, called by showConceptDetailPopup
     ...
     if (!loreData || !Array.isArray(loreData) || loreData.length === 0) { popupLoreSection.classList.add('hidden'); return; }
     popupLoreSection.classList.remove('hidden'); // Ensure visible if data exists
    ...
    loreData.forEach(entry => {
        if (entry.level <= unlockedLevel) { // Display unlocked lore
            ...
            const cost = entry.insightCost || Config.LORE_UNLOCK_COSTS[`level${entry.level}`] || 999; const canAfford = State.getInsight() >= cost;
...
// **NEW**: Internal helper for displaying new card unlocks in popup
function displayPopupCardUnlocks(conceptId, discoveredData) {
    ...
    const unlockState = discoveredData.unlocks || createDefaultCardUnlockState(); // Use imported creator
    ...
    const createButtonHTML = (key, icon, name, levelReq) => {
        if (elementLevel >= levelReq && !unlockState[key]?.unlocked) {
            ...
            return `<button class="button small-button unlock-button card-unlock-button btn" data-unlock-key="${key}" data-concept-id="${conceptId}" data-cost="${cost}" ${!canAfford ? 'disabled' : ''} title="${canAfford ? `Unlock ${name} (${cost} Insight)` : `Requires ${cost} Insight`}"> <i class="fas ${icon}"></i> Unlock ${name} (${cost}) </button>`;
...
export function showScreen(screenId) { const currentState = State.getState(); const isPostQuestionnaire = currentState.questionnaireCompleted; const targetScreenElement = getElement(screenId); if (!targetScreenElement) { screens.forEach(screen => screen?.classList.add('hidden', 'current')); const welcomeEl = getElement('welcomeScreen'); if(welcomeEl) { welcomeEl.classList.remove('hidden'); welcomeEl.classList.add('current'); } screenId = 'welcomeScreen'; } else { screens.forEach(screen => { if (screen) { screen.classList.add('hidden'); screen.classList.remove('current'); } }); targetScreenElement.classList.remove('hidden'); targetScreenElement.classList.add('current'); } updateDrawerLinks(); if (sideDrawer) sideDrawer.querySelectorAll('.drawer-link[data-target]').forEach(button => button?.classList.toggle('active', button.dataset.target === screenId)); try { switch (screenId) { case 'personaScreen': if (isPostQuestionnaire) { const justFinishedQuestionnaire = previousScreenId === 'questionnaireScreen'; const showDetailed = personaDetailedView?.classList.contains('current') ?? true; if (justFinishedQuestionnaire) togglePersonaView(false); else { if (showDetailed) GameLogic.displayPersonaScreenLogic(); else displayPersonaSummary(); } displayInsightLog(); updateXPDisplay(); // **NEW**: Update XP on showing Persona screen } else { showScreen('welcomeScreen'); return; } break; case 'workshopScreen': if (isPostQuestionnaire) { displayWorkshopScreenContent(); refreshGrimoireDisplay(); showHintOnce('freeResearch', '#freeResearchButtonWorkshop', '<b>Free research!</b><br>Click once per day to draw new cards.'); } else { showScreen('welcomeScreen'); return; } break; case 'repositoryScreen': if (isPostQuestionnaire) displayRepositoryContent(); else { showScreen('welcomeScreen'); return; } break; case 'questionnaireScreen': if (!isPostQuestionnaire) { const index = State.getState().currentElementIndex; if (index >= 0 && index < elementNames.length) displayElementQuestions(index); else initializeQuestionnaireUI(); } else { showScreen('personaScreen'); return; } break; case 'welcomeScreen': updateDrawerLinks(); break; } } catch (error) { console.error(`Error during display logic for screen ${screenId}:`, error); } if (['questionnaireScreen', 'workshopScreen', 'personaScreen', 'repositoryScreen'].includes(screenId)) window.scrollTo(0, 0); previousScreenId = screenId; }
 function toggleDrawer () { if (!sideDrawer || !drawerToggle) return; const willOpen = !sideDrawer.classList.contains('open'); if (willOpen) { sideDrawer.classList.add('open'); drawerToggle.setAttribute('aria-expanded', 'true'); sideDrawer.setAttribute('aria-hidden', 'false'); } else { drawerToggle.focus(); sideDrawer.classList.remove('open'); drawerToggle.setAttribute('aria-expanded', 'false'); sideDrawer.setAttribute('aria-hidden', 'true'); } const onboardingActive = onboardingOverlay && onboardingOverlay.classList.contains('visible'); if (popupOverlay && !onboardingActive) popupOverlay.classList.toggle('hidden', willOpen === false); }
...
export function updateXPDisplay(){
   ...
   // Get current XP and Level status
   const elementXP = State.getElementXP(); // { A: 120, I: 50, ... }
   const elementLevels = State.getElementLevel(); // { A: 1, I: 0, ... }

   // Calculate total XP accumulated *within the current levels* for bar visualization
   let totalXPWithinLevels = 0;
   let totalXPNeededForNextToken = 0; // Represents the 'total length' of the bar if we sum up progress towards next levels

   for (const key in elementLevels) {
        const currentLevel = elementLevels[key];
        const currentElementXP = elementXP[key] || 0;

        if (currentLevel < Config.MAX_ELEMENT_LEVEL) {
            const xpForCurrentLevel = Config.XP_LEVEL_THRESHOLDS[currentLevel]; // XP required *to reach* this level
            const xpForNextLevel = Config.XP_LEVEL_THRESHOLDS[currentLevel + 1];
            const xpInThisLevel = currentElementXP - xpForCurrentLevel; // How much XP earned since last level up
            const xpNeededForThisLevel = xpForNextLevel - xpForCurrentLevel; // Total XP span of this level

            totalXPWithinLevels += Math.max(0, xpInThisLevel); // Add progress within the current level
            totalXPNeededForNextToken += xpNeededForThisLevel; // Add the total size of this level bracket
        } else {
            // If max level, contribute the full XP needed for the *last* level bracket to the 'total'
            const xpForMaxLevel = Config.XP_LEVEL_THRESHOLDS[Config.MAX_ELEMENT_LEVEL];
            const xpForPrevLevel = Config.XP_LEVEL_THRESHOLDS[Config.MAX_ELEMENT_LEVEL - 1];
            const lastLevelBracketSize = xpForMaxLevel - xpForPrevLevel;
            totalXPWithinLevels += lastLevelBracketSize; // Assume max level is 'full' for bar calc
            totalXPNeededForNextToken += lastLevelBracketSize;
        }
   }

    // Calculate percentage for the bar
    const percentage = (totalXPNeededForNextToken > 0) ? (totalXPWithinLevels / totalXPNeededForNextToken) * 100 : 0;
   ...
   // Also update token count inside Sanctum modal if it exists
    if(sanctumTokenCountSpan) {
        sanctumTokenCountSpan.textContent = tokenCount;
    }
}

// --- Questionnaire UI ---
export function initializeQuestionnaireUI() { State.updateElementIndex(0); displayElementQuestions(0); if (dynamicScoreFeedback) dynamicScoreFeedback.style.display = 'none'; const totalElements = elementNames.length; document.documentElement.style.setProperty('--progress-pct', `${(1 / totalElements) * 100}%`); }
...
export function renderCard(concept, context = 'grimoire', discoveredData = null) {
    ...
    if(isDiscovered && unlockState) {
        ...
        if(unlockState.crossover?.unlocked && unlockState.crossover?.completed) unlockIndicatorsHTML += `<i class="fas fa-check-circle unlock-stamp" title="Crossover Challenge Complete!"></i>`; // Indicate completion
...
export function showConceptDetailPopup(conceptId) {
    ...
    // Legacy Lore Section
    displayPopupLore(conceptId, concept.lore, discoveredData.unlockedLoreLevel || 0);
    // Hide legacy lore if new system is active and has content
    if (popupCardUnlocksSection && !popupCardUnlocksSection.classList.contains('hidden') && popupCardUnlocksContent && popupCardUnlocksContent.innerHTML.trim() !== '' && popupCardUnlocksContent.innerHTML.trim() !== '<p><i>No unlocks currently available or all purchased.</i></p>') {
        if (popupLoreSection) popupLoreSection.classList.add('hidden');
    }
...
export function showSanctumModal(){
    if (!sanctumModal || !sanctumGrid || !sanctumTokenCountSpan) {
    ...
    sanctumTokenCountSpan.textContent = tokensAvailable; // Update token count inside modal
...
console.log("ui.js loaded successfully. (XP/Leveling v1.0 - Reconstructed)");
// --- END OF FILE ui.js ---
